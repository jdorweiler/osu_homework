...
void _addLinkAfter(struct cirListDeque *q, struct DLink *lnk, TYPE v)
{
    //check to see if q is null, lnk is null, and since we only use add
    // front and addBack check to see if lnk is the sentinel or the linke
    // before the sentinel.
    assert((q != 0 && lnk !=0) && (lnk == q->Sentinel || lnk == q->Sentinel->prev));
    ...
    //check to see if this is the first link
    //added in the buffer or adding to back
    if(q->size == 0 || temp->next == q->Sentinel){
        q->Sentinel->prev = temp;
    }
    // check to see if adding to front
    if(lnk = q->Sentinel){
        temp->next->prev = temp;
    }
    q->size++;
