...
void addHeap(DynArr *heap, TYPE val)
{
    ...
    addDynArr(heap, val); /* add val to end of the array, resize if needed */
...
void removeMinHeap(DynArr *heap)
{
   ...
   last = sizeDynArr(heap) - 1; /*index of last element*/
   putDynArr(heap, 0, getDynArr(heap, last)); /*move last to top of heap*/
...
{
    assert(!isEmptyDynArr(heap));
    int lastLeaf = (sizeDynArr(heap) -1) / 2;
    int last = sizeDynArr(heap);
    while(lastLeaf != -1){
        _adjustHeap(heap, last, lastLeaf);
        --lastLeaf;
    }
...
void sortHeap(DynArr *heap)
{
    ...
    while(pos != -1){
        swapDynArr(heap, pos, 0);
        _adjustHeap(heap, pos, 0);
        --pos;
    }
}
