...
void print_histogram(std::map<int, int> data,
                     ...
                     int totalSides,
                     int rolls); // funciton that prints out a histogram to the screen
...
void makeColors(std::map<int, std::string> &colors, int totalSides){
    ...
    for(int i = 0; i < totalSides; ++i){
        int colorCode = i%6+1;
        // static cast to long long so it works on flip.. yay outdated compilers.
